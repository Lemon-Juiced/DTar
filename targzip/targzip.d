 ...
 * - output (optional): Destination path for the tarball. Defaults to `<path>.tar.gz`.
 * 
...
void main(string[] args) {
    ...
    if (args.length < 2 || args.length > 3) {
        writeln("Usage: targzip <path> [output]");
        return;
    }
    ...
    else output = path ~ ".tar.gz"; // If no output is provided, use the current working directory and the name of the input path
    ...
    // Create the tarball using the tar command with gzip compression
    string command = "tar -cvzf " ~ output ~ " " ~ path;
    try {
