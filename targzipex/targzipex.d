...
void main(string[] args) {
    ...
    if (args.length < 2 || args.length > 3) {
        writeln("Usage: targzipex <archive> [path]");
        return;
    }
    ...
    // Extract the contents of the tarball using the tar command with gzip decompression
    string command = "tar --strip-components=1 -xvzf " ~ archive ~ " -C " ~ path;
    try {
